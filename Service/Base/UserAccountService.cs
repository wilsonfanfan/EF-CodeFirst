...
namespace X.Service
{
    public partial class UserAccountService : AppServiceBase, IUserAccountService
    {
		public IUserAccountDao repository { get; set; } 
		
        public UserAccountService(IUserAccountDao _repository)
        {
        ...
        public IResponse<Models.UserAccount> Create(Models.UserAccount model)
        {
            ...
            if (entity == null)
            {
                return new IResponse<Models.UserAccount> { Status = false, ErrCode = ErrorMessage.DataNotExist.Code, ErrMsg = ErrorMessage.DataNotExist.Msg };
            }
            return new IResponse<Models.UserAccount> { Status = true, Result = Mapper.Map<Models.UserAccount>(entity) };
        }
        public IListResult<Models.UserAccount> GetList(string Filter = null, string Sort = null, params object[] Values)
        {
            ...
            var list = query.ToList().Select(entity => Mapper.Map<Models.UserAccount>(entity)).ToList();
            var response = new IListResult<Models.UserAccount> { Status = true, Result = list };
            return response;
        }

        public IPageResult<Models.UserAccount> GetPageList(string Filter, string Sort, int PageNumber = 1, int PageSize = 20, params object[] Values)
        {
            var query = repository.Query();
            if (!Filter.IsNull())
            {
            ...
            long count = 0;
            var items = query.Page(PageNumber, PageSize, out count).ToList().Select(entity => Mapper.Map<Models.UserAccount>(entity)).ToList();

            var response = new IPageResult<Models.UserAccount>()
            {
                Result = new PagedResult<Models.UserAccount>
                {
        ...
        public IResponse<Models.UserAccount> GetById(long ID)
        {
            var response = new IResponse<Models.UserAccount>
            {
                Status = true,
                Result = Mapper.Map<Models.UserAccount>(repository.GetById(ID))
            };
            return response;
        }
        public IResponse<Models.UserAccount> Get(string Filter, params object[] values)
        {
            var query = repository.Query();
            if (!Filter.IsNull())
            {
            ...
            {
                Status = true,
                Result = Mapper.Map<Models.UserAccount>(query.First())
            };
            return response;
        }
